...
class ClientFragment : Fragment(), CarAdapter.Listener {
    ...
    override fun onCreateView(
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                val response = service.getCarsByClientPhone(user.phone)
                if(response.isSuccessful) {
                    response.body()?.let { InitRC(it) }
                }
                else {
                    binding.textWelcomeCars.text = "Вы еще не приобрели у нас ни одной машины"
                }
            }
        }
    ...
    fun InitRC(cars: List<Car>){
        binding.apply {
            ...
            rcView.adapter = CarAdapter(this@ClientFragment, cars)
        }
    }
    ...
    override fun onClick(car: Car) {
        startActivity(Intent(requireContext(), CarDetailActivity::class.java).apply {
